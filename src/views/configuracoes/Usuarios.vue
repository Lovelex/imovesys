  ...
  <div>
    <v-container fluid>
      <DialogContainer :dialog="dialog">
        <UsuariosTabs :tab="tab" :mode="mode" :usuario="usuario" />
        <div class="d-flex justify-end">
          <v-btn v-if="mode === 'submit'" color="primary">Adicionar</v-btn>
          <v-btn v-if="mode === 'update' && tab.number !== 1" color="warning">Editar</v-btn>
          <v-btn @click="toggleDialog" color="secondary" class="ml-2">Fechar</v-btn>
        </div>
      </DialogContainer>
...
import UsuariosTabs from "@/components/configuracoes/usuarios/UsuariosTabs";
import dialog from "@/mixins/dialog";

import { mapGetters, mapActions } from "vuex";

export default {
  components: { DialogContainer, UsuariosTabs },
  mixins: [dialog],
  data: () => ({
    mode: "submit",
    tab: {
